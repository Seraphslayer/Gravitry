...
const droppedPinIcon = L.divIcon({
  ...
  html: `
    <div style="position:relative;width:30px;height:30px;display:flex;align-items:center;justify-content:center;">
      <div style="width:14px;height:14px;border-radius:50%;background:#DC2626;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.4);"></div>
    </div>`,
  iconSize: [30, 30],
  iconAnchor: [15, 15],
});
...
function SlipRow({ label, value, icon, mono, accent }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, color:C.muted, fontSize:12, fontFamily:IN }}>
        {icon} {label}
      </div>
      <span style={{ fontFamily:mono?"monospace":accent?GR:IN, fontWeight:accent?700:500,
        fontSize:accent?16:13, color:accent?C.green:C.text }}>
        {value}
      </span>
    </div>
  );
...
export default function KioskView({ dispatchLog, pendingRequests, setPendingRequests }) {
  ...
  const pickLocation = (lat, lng) => {
    ...
    const s = nearestTerminal(lt, lg, KIOSK);
    setSnap(s);
    setPinMode(false);
    setResults([]);
    setQuery("");
  ...
  const confirmSnap = () => {
    if (snap?.terminal) { setDest(snap.terminal); setStep("fare"); }
  };
    ...
    <div className="kiosk-shell">
      <div className="kiosk-map-zone">
        ...
        <div style={{ position:"absolute", top:"calc(12px + var(--safe-top))", left:12,
          ...
          <span style={{ color:"#fff", fontFamily:GR, fontWeight:700, fontSize:13, letterSpacing:0.3 }}>GRAVITRY</span>
        </div>

        <div style={{ position:"absolute", top:"calc(12px + var(--safe-top))", right:12,
          zIndex:400, display:"flex", alignItems:"center", gap:6,
          background:"rgba(255,255,255,0.95)", padding:"7px 12px", borderRadius:20,
          boxShadow:"0 4px 16px rgba(0,0,0,0.2)" }}>
          <div style={{ width:7, height:7, borderRadius:"50%", background:C.green }} />
          <span style={{ fontFamily:GR, fontWeight:600, fontSize:11, color:C.text }} className="hide-xs">
            {originTerm.short}
          </span>
        </div>
        {pinMode && (
          <div style={{ position:"absolute", bottom:12, left:12, right:12, zIndex:400,
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <span style={{ color:"#fff", fontFamily:IN, fontSize:12.5 }}>Tap the map to drop a pin</span>
            </div>
            <button onClick={() => setPinMode(false)}
              style={{ background:"rgba(255,255,255,0.12)", border:"none", borderRadius:8,
                padding:"5px 10px", color:"#fff", fontFamily:GR, fontSize:11.5, cursor:"pointer" }}>
              Cancel
            </button>
          </div>
        )}
      ...
      <div className="bottom-sheet">
        ...
        {step === "home" && (
          <div>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
              <div>
                <h1 style={{ fontFamily:GR, fontWeight:700, fontSize:"clamp(18px, 5vw, 22px)", color:C.text, margin:0 }}>
                  Need a ride?
                </h1>
                <p style={{ fontFamily:IN, fontSize:13, color:C.muted, margin:"2px 0 0" }}>
                  Official TODA dispatch · Verified drivers
                </p>
              </div>
              <div style={{ width:44, height:44, borderRadius:12, background:C.greenLight,
            ...
            <button onClick={() => setStep("select")}
              style={{ width:"100%", marginTop:18, padding:"18px 20px", borderRadius:16, border:"none",
                background:C.yellow, color:C.navy, fontSize:"clamp(15px, 4vw, 17px)", fontWeight:700, fontFamily:GR,
            ...
            <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap" }}>
              <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11.5, color:C.muted, fontFamily:IN }}>
        ...
        {step === "select" && (
          <div>
            <h2 style={{ fontFamily:GR, fontWeight:700, fontSize:"clamp(16px, 4.5vw, 19px)", color:C.text, margin:"0 0 2px" }}>
              Where are you going?
            </h2>
            <p style={{ fontFamily:IN, fontSize:12.5, color:C.muted, margin:"0 0 14px" }}>
            ...
            <div style={{ position:"relative", marginBottom:10 }}>
              ...
              <input
                ...
                style={{ width:"100%", padding:"13px 36px", borderRadius:14, border:`1.5px solid ${C.border}`,
                  fontFamily:IN, fontSize:14, outline:"none", background:C.surface }}
              />
              {query && (
            ...
            {/* Search results */}
            {searching && <p style={{ fontSize:12, color:C.muted, fontFamily:IN, margin:"0 0 10px" }}>Searching…</p>}
            {results.length > 0 && (
              <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:14 }}>
                {results.map((r, i) => (
                  <button key={i} onClick={() => pickLocation(r.lat, r.lon)}
                    style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12,
                      padding:"10px 14px", textAlign:"left", cursor:"pointer", display:"flex",
                      alignItems:"center", gap:10 }}>
                    <MapPin size={14} color={C.muted} style={{ flexShrink:0 }} />
                    <span style={{ fontFamily:IN, fontSize:12.5, color:C.text, lineHeight:1.3 }}>{r.display_name}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            {!snap && (
              <button onClick={() => setPinMode(true)}
                style={{ width:"100%", marginBottom:16, padding:"12px 16px", borderRadius:14,
                  border:`1.5px dashed ${C.green}`, background:C.greenLight, color:C.greenDark,
                  fontFamily:GR, fontWeight:600, fontSize:13.5, cursor:"pointer",
                  display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                <Crosshair size={16} /> Drop a pin on the map instead
              </button>
            )}

            {/* Snap-to-nearest-terminal confirmation */}
            {snap?.terminal && (
              <div style={{ background:C.navy, borderRadius:14, padding:"14px 16px", marginBottom:14 }}>
                <div style={{ color:"rgba(255,255,255,0.5)", fontFamily:IN, fontSize:11, marginBottom:6 }}>
                  Nearest terminal to your pin ({Math.round(snap.distanceMeters)}m away):
                </div>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <div>
                    <div style={{ color:"#fff", fontFamily:GR, fontWeight:700, fontSize:15 }}>{snap.terminal.name}</div>
                    <div style={{ color:C.yellow, fontFamily:GR, fontSize:13, fontWeight:600 }}>
                      ₱{getFare(KIOSK, snap.terminal.id)} official fare
                    </div>
                  </div>
                  <button onClick={confirmSnap}
                    style={{ background:C.yellow, color:C.navy, border:"none", borderRadius:10,
                      padding:"9px 16px", fontFamily:GR, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                    Use this
                  </button>
                </div>
                <button onClick={() => { setSnap(null); setDroppedPin(null); }}
                  style={{ marginTop:8, background:"transparent", border:"none", color:"rgba(255,255,255,0.4)",
                    fontFamily:IN, fontSize:12, cursor:"pointer", padding:0 }}>
                  Try a different spot
                </button>
              </div>
            )}

            <p style={{ fontSize:11, color:C.muted, fontFamily:IN, margin:"0 0 8px" }}>Or pick a terminal directly:</p>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {dests.map(t => (
                <button key={t.id} onClick={() => { setDest(t); setStep("fare"); }}
                  style={{ background:C.surface, border:`1.5px solid ${C.border}`, borderRadius:14,
                    padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"space-between",
                    cursor:"pointer", textAlign:"left" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ width:36, height:36, borderRadius:10, background:C.greenLight,
                      display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                      <MapPin size={16} color={C.green} />
                    </div>
                    <div>
                      <div style={{ color:C.text, fontFamily:GR, fontWeight:600, fontSize:14 }}>{t.name}</div>
                      <div style={{ color:C.muted, fontFamily:IN, fontSize:12 }}>₱{getFare(KIOSK, t.id)} official fare</div>
                    </div>
                  </div>
            ...
            <button onClick={() => { setStep("home"); setQuery(""); setResults([]); setSnap(null); setDroppedPin(null); }}
              style={{ marginTop:14, background:"transparent", border:"none", color:C.muted,
                fontFamily:IN, fontSize:13.5, cursor:"pointer", padding:"8px 0", width:"100%" }}>
              ← Back
            </button>
          </div>
        )}
        ...
        {step === "fare" && dest && (
          <div>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10,
              marginBottom:16, fontFamily:IN, fontSize:13 }}>
              <span style={{ color:C.text, fontWeight:600 }}>{originTerm.short}</span>
              <ArrowRight size={14} color={C.muted} />
              <span style={{ color:C.green, fontWeight:600 }}>{dest.short}</span>
            </div>

            <div style={{ background:C.navy, borderRadius:18, padding:"22px 20px", textAlign:"center", marginBottom:16 }}>
              <div style={{ color:"rgba(247,195,68,0.6)", fontFamily:GR, fontSize:10,
                letterSpacing:2, textTransform:"uppercase", marginBottom:8 }}>
                TODA-Approved Official Fare
              </div>
              <div className="fare-display-amount" style={{ color:C.yellow, fontFamily:GR, fontWeight:700,
                lineHeight:1, fontSize:"clamp(48px, 14vw, 64px)" }}>
                ₱{fare}<span style={{ fontSize:"0.35em", opacity:0.5, fontWeight:400 }}>.00</span>
              </div>
              <div style={{ marginTop:10, color:"rgba(255,255,255,0.4)", fontFamily:IN, fontSize:11.5, lineHeight:1.5 }}>
            ...
            <button onClick={handleConfirm}
              style={{ width:"100%", padding:"16px 0", borderRadius:14, border:"none",
                background:C.green, color:"#fff", fontSize:15, fontWeight:700, fontFamily:GR,
                cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center",
                gap:8, marginBottom:8, boxShadow:"0 4px 16px rgba(28,124,84,0.35)" }}>
              <CheckCircle size={18} /> Confirm & Request Trip
            </button>
            <button onClick={() => setStep("select")}
        ...
        {step === "success" && record && (
          <div>
            <div style={{ textAlign:"center", marginBottom:14 }}>
              ...
              <p style={{ fontFamily:IN, fontSize:12.5, color:C.muted, margin:0 }}>
            ...
            <div style={{ background:C.surface, borderRadius:14, padding:16, border:`1px solid ${C.border}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                <span style={{ fontFamily:GR, fontWeight:700, fontSize:12, color:C.navy }}>Passenger Safety Record</span>
                <Shield size={16} color={C.green} />
              </div>
              <SlipRow icon={<User size={13}/>}  label="Driver"    value={record.driver} />
              <SlipRow icon={<Truck size={13}/>} label="Unit No."  value={record.unit}   mono />
              <SlipRow icon={<Hash size={13}/>}  label="Plate No." value={record.plate}  mono />
              <SlipRow icon={<span style={{ fontSize:12 }}>₱</span>} label="Fare" value={`₱${record.fare}.00`} accent />
              <div style={{ marginTop:10, paddingTop:10, borderTop:`1px dashed ${C.border}`,
                display:"flex", justifyContent:"space-between", fontSize:11, color:C.muted, fontFamily:IN }}>
                <span>{record.id} · {fmtTime(record.ts)}</span>
                <span style={{ fontWeight:700, color:C.green, fontFamily:GR }}>VERIFIED ✓</span>
              </div>
            </div>

            <button onClick={reset}
              style={{ marginTop:12, width:"100%", padding:"13px 0", background:C.surface,
                border:`1px solid ${C.border}`, borderRadius:12, color:C.muted, fontFamily:GR,
